          ...
          <Grid container spacing={10}>
            <Grid item xs={7} sm={7}>
              <Card className={classes.card}>
                <CardHeader
                  title={product.name}
                  subheader={product.quantity > 0? 'Є в наявності': 'немає в наявності'}
                  action={
                ...
                <div className={classes.flex}>
                  ...
                  <Typography component="p" variant="subtitle1" className={classes.subheading}>
                    ...
                    <span className={classes.price}>{product.price},00 грн</span>
                    <Link to={'/shops/'+product.shop._id} className={classes.link}>
                      <span>
                        <Icon className={classes.icon}>Повернутися до магазину  </Icon> {product.shop.name}
                      </span>
                    </Link>
                  </Typography>
            ...
            {suggestions.length > 0 &&
              (<Grid item xs={5} sm={5}>
                <Suggestions  products={suggestions} title='Схожі Товари'/>
              </Grid>)}
          </Grid>
        </div>)
}
